    ...
    execute: function () {
      ...
      _export("PanShake", PanShake = (_dec = ccclass('PanShake'), _dec2 = property(Node), _dec(_class = (_class2 = class PanShake extends Component {
        ...
        onPanShakeBtnClick() {
          ...
          for (var child of this.ballParent.children) {
            var rigid = child.getComponent(RigidBody);
            if (!rigid) continue; // 1. 施加向上抛的线性冲量

            var randX = (Math.random() - 0.5) * this.horizontalForce;
            var randZ = (Math.random() - 0.5) * this.horizontalForce;
            var impulse = new Vec3(randX, this.jumpForce, randZ);
            rigid.applyImpulse(impulse); // 2. 施加随机旋转扭矩，升空自带翻滚

            var torque = new Vec3((Math.random() - 0.5) * this.rotateTorque, (Math.random() - 0.5) * this.rotateTorque * 0.5, (Math.random() - 0.5) * this.rotateTorque);
...
//# sourceMappingURL=8c734b542d532d30b03ac635443247cbfcb16744.js.map